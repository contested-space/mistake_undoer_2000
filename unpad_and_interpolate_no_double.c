...
int main( int argc, char *argv[] ){
	...
	while (i<nbLine){
		...
		if (!(noDoubleArrayLeftSample[i] == 0 || noDoubleArrayRightSample[i] == 0)){
			newtrackArrayTime[j] = noDoubleArrayTime[i];
			newtrackArrayLeftSample[j] = noDoubleArrayLeftSample[i];
			newtrackArrayRightSample[j] = noDoubleArrayRightSample[i];
			i++;
			j++;
		}
		else {
			...
			while (noDoubleArrayLeftSample[i] == 0 || noDoubleArrayRightSample[i] == 0){
				i++;
			}
